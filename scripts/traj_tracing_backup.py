...
path_to_init = os.path.dirname(os.path.abspath(__file__)) + '/../my_init_positions'
path_to_episodes = os.path.dirname(os.path.abspath(__file__)) + '/../episodes'
path_to_trajs = os.path.dirname(os.path.abspath(__file__)) + '/../my_traj_files'
...
class TraceALine:
        ...
        if initial_ik is not None:
            ...
            if True:
                ...
                for ik in initial_ik:
                    conf = list(movo_jointangles_fik2rik(ik, grippber_value=0.9))
        ...
        for i in range(num_keypoints - 1):
            ...
            if self.start_from_init_pose and i == 0:
                ...
                for arm_idx in range(self.robot.num_chain):
                    upd[f"envcollision_{arm_idx}"] = 10
                
                
                weight_updates.append(upd)
                num_empty_updates -= 1
            elif self.start_from_init_pose and i == 6:
    ...
    def get_ik_list_from_traj(self) -> list:
        ...
        for j in range(len(self.trajectory)):
            ...
            print("x y z differences:", gt_positions - ik_positions)
            ik_solutions.append(ik_solution)
            losses = self.ik_solver.query_loss(ik_solution)
            print(f"Total Loss: {sum(losses)}")
            # print(self.get_individual_loss(losses, ['eepos', "eequat", "selfcollision_ee"]))
            print(j)
...
if __name__ == '__main__':
    ...
    if args.no_ros:
        ...
        np.save(f"{fpath}/ik_seq{trace_a_line.episode}_{trace_a_line.side}_{trace_a_line.left_map_id}_{trace_a_line.right_map_id}.npy", ik_arr)
